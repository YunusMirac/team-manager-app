...
app.post("/invite", jwtVerify, requireChefRole, async (req, res) => {
    ...
    try {
        ...
        // Upsert in DB
        await connection.query(`INSERT INTO users (email, pin, pin_expires, role) 
       VALUES ($1, $2, $3, 'employee')
       ON CONFLICT (email) DO UPDATE 
       SET pin = EXCLUDED.pin, pin_expires = EXCLUDED.pin_expires`, [email, pin, expiresAt]);
        // E-Mail versenden
...
app.post("/validate-invite", async (req, res) => {
    ...
    try {
        ...
        const exists = (await connection.query("SELECT 1 FROM users WHERE email = $1 AND password IS NOT NULL", [email]));
        ...
        await connection.query("BEGIN");
        const result = await connection.query(`SELECT pin_expires FROM users 
       WHERE pin = $1 AND email = $2 AND pin_expires >= NOW()
       FOR UPDATE`, [pin, email]);
        ...
        await connection.query("UPDATE users SET pin = NULL, pin_expires = NULL WHERE email = $1", [email]);
        await connection.query("COMMIT");
...
app.post("/signup", async (req, res) => {
    try {
        ...
        const updateResult = await pool.query("UPDATE users SET username = $1, password = $2 WHERE email = $3", [username, hashedPassword, email]);
        ...
        console.log(`User ${email} erfolgreich registriert mit Username ${username}`);
...
app.post("/login", async (req, res) => {
    try {
        ...
        const result = await pool.query("SELECT username, email, password, role FROM users WHERE email = $1 OR username = $2", [email, username]);
...
app.get("/users", async (req, res) => {
    try {
        const result = await pool.query("SELECT email, username, role, created_at FROM users ORDER BY created_at");
...
app.post("/tasks-unassign-user", async (req, res) => {
    ...
    try {
        ...
        await pool.query("UPDATE tasks SET status = 'offen', assigned_to = NULL WHERE assigned_to = $1 AND status = 'in bearbeitung'", [email]);
        // 2. Aufgaben "fertig" → assigned_to = NULL (Status bleibt "fertig")
        await pool.query("UPDATE tasks SET assigned_to = NULL WHERE assigned_to = $1 AND status = 'fertig'", [email]);
...
function verifyToken(req, res, next) {
    ...
    try {
        ...
        req.user = payload; // z.B. { id, email, role, iat, exp }
        next();
    }
    catch (err) {
...
app.get("/tasks-user", verifyToken, async (req, res) => {
    const email = req.query.email;
    try {
        const result = await pool.query("SELECT * FROM tasks WHERE assigned_to = $1 ORDER BY due_date", [email]);
        res.json(result.rows);
    }
    catch (error) {
...
function requireChefRole(req, res, next) {
    const user = req.user; // falls `req.user` nicht im Typ bekannt ist
...
app.post("/add-tasks", jwtVerify, requireChefRole, async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO tasks (title, description, due_date)
       VALUES ($1, $2, $3) RETURNING *`, [title, description, due_date]);
...
app.get("/tasks-unassigned", async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM tasks WHERE assigned_to IS NULL ORDER BY due_date ASC");
...
app.get("/tasks-unassigned-newest", async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM tasks WHERE assigned_to IS NULL ORDER BY created_at DESC");
        res.json({ success: true, tasks: result.rows });
    }
    catch (err) {
...
app.get("/tasks-open", async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM tasks WHERE status = $1 ORDER BY due_date ASC", ["offen"]);
        res.json({ success: true, tasks: result.rows });
    }
    catch (err) {
